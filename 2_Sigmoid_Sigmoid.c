...
double Zi[90][7],firstsigmoid[90][7],testZi[10][7],testfirstsigmoid[10][7]; //Sigmoid Activation for Training & Testing
...
void Sigmoid_1_Activation();//First forward propagation from Input to Hidden layer using Sigmoid Activation
void Sigmoid_2_Activation();//Second forward propagation from Hidden to Output layer using Sigmoid Activation
...
void Sigmoid_1_Activation()
{
    for(int a=0; a < 7 ; a++) //7 Hidden layer neurons
    {
        for(int i = 0; i < 90; i++) //90 volunteers
        {
            ...
            Zi[i][a] += hiddenbias[a];
            firstsigmoid[i][a] = 1/(1+exp(-1*Zi[i][a]));
        }
    }
}

void Sigmoid_2_Activation()
{
    for(int i = 0; i<90;i++)//90 Volunteers
    {
        ...
        for(int a=0 ; a<7 ; a++)
        {
            outputZi[i] += (outputweight[a]*firstsigmoid[i][a]);
        }
    outputZi[i] += bias;
...
void BackProp()
{
    for (int a = 0; a < 7; a++)
    {
        ...
        for (int i = 0; i < 90; i++)
        {
            derivativeow[a] += (sigmoid[i]-Training[i][9])*(exp(outputZi[i])/pow(1+exp(outputZi[i]),2))*firstsigmoid[i][a];//Derivative of the weight
        }
        derivativeow[a] /= 90;
    }
...
void HiddenBackProp()
{
    for (int a =0;a<7;a++)
    {
        for (int j = 0; j < 9; j++) 
        {
            ...
            {
                {
                derivativehw[j][a] += (sigmoid[i]-Training[i][9])*(exp(outputZi[i])/pow(1+exp(outputZi[i]),2))*outputweight[a]*(exp(Zi[i][a])/pow(1+exp(Zi[i][a]),2))*Training[i][j];//Derivative of weight
                }
            }
            derivativehw[j][a] /= 90;
        }
    ...
    for (int i = 0; i < 90; i++)
    {
        derivativehb[a] += (sigmoid[i]-Training[i][9])*(exp(outputZi[i])/pow(1+exp(outputZi[i]),2))*outputweight[a]*(exp(Zi[i][a])/pow(1+exp(Zi[i][a]),2));//Derivative of bias
    }
    derivativehb[a] /= 90;
    }
}
...
void MachineTraining()
{
    ...
    do
    {    
    Sigmoid_1_Activation();
    Sigmoid_2_Activation();
    if (iteration == 1)
    {
        GetMMSE();
    }
...
void GetTesting()
{
    for(int a=0; a < 7 ; a++) //7 hidden neurons
    {
        for(int i = 0; i < 10; i++) //10 volunteers
        {
            ...
            testfirstsigmoid[i][a] = 1/(1+exp(-1*testZi[i][a]));
        }
    }
    for(int i = 0; i< 10;i++)
    {
        ...
        for(int a=0 ; a<7 ; a++)
        {
            testoutputZi[i] += (outputweight[a]*testfirstsigmoid[i][a]);
        }
    testoutputZi[i] += bias;
