...
double Zi[90][7],relu[90][7],drelu[90][7],testZi[10][7],testrelu[10][7]; //ReLu Activation for Training & Testing
...
void ReluActivation();//First forward propagation from Input to Hidden layer using Relu Activation
...
void ReluActivation()
{
    for(int a=0; a < 7 ; a++) //7 Hidden layer neurons
    {
        for(int i = 0; i < 90; i++) //90 volunteers
        {
            ...
            Zi[i][a] += hiddenbias[a];
            relu[i][a] = Zi[i][a]>0?Zi[i][a]:0;
            drelu[i][a] = Zi[i][a]>0?1:0;
        }
    }
}
...
void SigmoidActivation()
{
    for(int i = 0; i<90;i++)//90 Volunteers
    {
        ...
        for(int a=0 ; a<7 ; a++)
        {
            outputZi[i] += (outputweight[a]*relu[i][a]);
        }
    outputZi[i] += bias;
...
void BackProp()
{
    for (int a = 0; a < 7; a++)
    {
        ...
        for (int i = 0; i < 90; i++)
        {
            derivativeow[a] += (sigmoid[i]-Training[i][9])*(exp(outputZi[i])/pow(1+exp(outputZi[i]),2))*relu[i][a];//Derivative of the weight
        }
        derivativeow[a] /= 90;
    }
...
void HiddenBackProp()
{
    for (int a =0;a<7;a++)
    {
        for (int j = 0; j < 9; j++) 
        {
            ...
            for (int i = 0; i < 90; i++)
            {
                ...
                derivativehw[j][a] += (sigmoid[i]-Training[i][9])*(exp(outputZi[i])/pow(1+exp(outputZi[i]),2))*outputweight[a]*drelu[i][a]*Training[i][j];//Derivative of weight
    ...
    for (int i = 0; i < 90; i++)
    {
        derivativehb[a] += (sigmoid[i]-Training[i][9])*(exp(outputZi[i])/pow(1+exp(outputZi[i]),2))*outputweight[a]*drelu[i][a];//Derivative of bias
    }
    derivativehb[a] /= 90;
    }
}
...
void GetTesting()
{
    for(int a=0; a < 7 ; a++) //7 hidden neurons
    {
        for(int i = 0; i < 10; i++) //10 volunteers
        {
            ...
            testrelu[i][a] = testZi[i][a]>0?testZi[i][a]:0; 
        }
    }
    for(int i = 0; i< 10;i++)
    {
        ...
        for(int a=0 ; a<7 ; a++)
        {
            testoutputZi[i] += (outputweight[a]*testrelu[i][a]);
        }
    testoutputZi[i] += bias;
